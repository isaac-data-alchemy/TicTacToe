...
class Board:
    ...
    def is_winner(self, symbol: str) -> bool:
        for i in range(self.size):
            if all(self.grid[i][j] == symbol for j in range(self.size)) or all(
                self.grid[j][i] == symbol for j in range(self.size)
            ):
                return True
        if all(self.grid[i][i] == symbol for i in range(self.size)) or all(
            self.grid[i][self.size - 1 - i] == symbol for i in range(self.size)
        ):
            return True
        return False
